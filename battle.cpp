...
void inizio(){
	...
	for (i=0;i < YGRIGLIA; i++){
		for (j = 0; j< XGRIGLIA; j++){
			computer.getCasella(j, i, appocchio);
			switch(appocchio){
				case VUOTO:
	...
	*****************FINEPROVA**************/
	...
	do{
		...
		if (puls == 1){			//se premuto il pulsante sinistro del mouse
			...
			if (zona == GRIGLIA){
				if (! sparato){		//se non ha ancora effettuato il colpo
					logX = x;
					logY = y;
...
void compFase(){
	...
	caselle appocchio;
	int p,j;
	for (p=0;p < YGRIGLIA; p++){
		for (j = 0; j< XGRIGLIA; j++){
			computer.getCasella(j, p, appocchio);
			switch(appocchio){
				case VUOTO:
					disegna.casella(j,p,"prova\\vuoto.bmp",0);
					break;
				case NAVE:
					disegna.casella(j,p,"prova\\nave.bmp",0);
					break;
				case SPRVUOTO:
					disegna.casella(j,p,"prova\\sprvuoto.bmp",0);
					break;
				case SPRNAVE:
					disegna.casella(j,p,"prova\\sprnave.bmp",0);
					break;
				case IMPOSS:
					disegna.casella(j,p,"prova\\imposs.bmp",0);
					break;

				default:
					disegna.casella(j,p,"prova\\errore.bmp",0);
					break;
			}
		}
	}
